...
void main()
{
  ...
  // Keep sky reflection visible without washing the water out to white.
  float envIntensity = clamp((envColor.r + envColor.g + envColor.b) * hdrMultiplier, 0.0, 0.30);
  envColor.rgb *= envIntensity;

  // Compute refraction ratio (Fresnel):
  float facing = 1.0 - dot(-E, N);
  float waterEnvRatio = clamp(pow(facing, fresnelPower) * 0.22, 0.0, 0.18);

  // Refracted ray only considers deep and shallow water colors:
  vec4 waterColor = mix(shallowColor, deepColor, clamp(facing, 0.0, 1.0));

  // Perform linear interpolation between reflection and refraction.
  vec4 color = mix(waterColor, envColor, waterEnvRatio);

  fragColor = vec4(color.xyz, 0.96);
